    ...
    var layout = {
        loadLayout: function() {
            ...
            $.each(variables.layoutData, function() {
                $(".divider").before('<li><a><span data-check class="check" style="visibility:' + ((variables.currentLayoutData.LayoutName == this.LayoutName) ? "visible" : "hidden") + ';"></span><span>' + this.LayoutName + '</span></a></li>');
            });
            $("#tbl_layoutList").empty();
            var $filterListHtml = $("<tbody></tbody>");
            $(variables.layoutData).each(function(index, item) {
                var $filterItem = $("<tr><td></td></tr>");
                $filterItem.find("td").append("<input id='chk_filter_" + index + "' value='" + item.LayoutName +
                    "' type='checkbox' name='chkFilterName' /><label for='chk_filter_" + index + "'>" + item.LayoutName + "</label>");
                $filterListHtml.append($filterItem);
            });
            $("#tbl_layoutList").append($filterListHtml);
        },
        initLayout: function() {
            $("#columnLayout").on("click", function(event) {
            ...
            $("#columnLayout ul").on("click", "a", function(event) {
                event.stopPropagation();
                $("#columnLayout").toggleClass("open");
                ...
                grid[variables.currentTabID].loadLayout();
            });
            $("a[data-action=addOrRemoveColumn]", "#columnLayout").on("click", function(event) {
                grid[variables.currentTabID].initColumnControlPanel();
                event.stopPropagation();
                $("#columnLayout").removeClass("open");
                $("#columnLayoutList").jqxWindow('open');
            });
            $("a[data-action=saveLayout]", "#columnLayout").on("click", function(event) {
                var $pop = $(variables.ids.windowSaveLayoutID);
                if (!$pop.data("jqxWindow")) {
                    $pop.jqxWindow({
                        autoOpen: false,
                        isModal: true,
                        height: 125,
            ...
            $("a[data-action=deleteLayout]", "#columnLayout").on("click", function(event) {
                var $deletePop = $(variables.ids.windowDeleteLayoutID);
                if (!$deletePop.data("jqxWindow")) {
                    $deletePop.jqxWindow({
                        autoOpen: false,
                        isModal: true,
                        width: 300
            ...
            $("a[data-action=resetLayout]", "#columnLayout").on("click", function(event) {
                layout.resetLayout();
            });
            ...
            $("#btnDeleteLayout").on("click", function(event) {
                ...
                $(variables.ids.windowDeleteLayoutID).jqxWindow('close');
            });
        },
        deleteLayout: function(layoutNames, callback) {
            ...
            $.ajax({
                type: "post",
                url: variables.urls.deleteLayout,
                dataType: "text",
                data: {
                    LayOutEntity: Base64.encode(JSON.stringify(layout))
                },
                cache: false,
                timeout: 1000 * 60,
                beforeSend: function() {
                    $(variables.rootID).mask("Deleting layout data...");
                },
                complete: function() {
                    $(variables.rootID).unmask();
                },
                success: function(response) {
                    ...
                    if (response.indexOf("true") < 0) {
                        ...
                        var newLayout = [];
                        $.each(variables.layoutData, function(i, val) {
                    ...
                    VantageSearchPopup.openMessagePopup(message);
                }
            });
        },
        saveLayout: function(layoutName, isDefault, existsLayout) {
            ...
            $.each(BaseGrid.ids, function(i, v) {
                layout.layoutCriteria[v] = $(v).jqxGrid('getstate');
            });
            $.ajax({
                type: "post",
                url: variables.urls.saveLayout,
                dataType: "text",
                data: {
                    LayOutEntity: Base64.encode(JSON.stringify(layout))
                },
                ...
                timeout: 1000 * 60,
                beforeSend: function() {
                    $(variables.rootID).mask("Saving layout data...");
                },
                complete: function() {
                    $(variables.rootID).unmask();
                },
                success: function(response) {
                    variables.currentLayoutData = layout;
                    variables.currentLayoutData.LayoutName = layout.layoutName;
                    variables.currentLayoutData.LayoutCriteria = layout.layoutCriteria;
                    existsLayout || variables.layoutData.push(variables.currentLayoutData);
        ...
        resetLayout: function() {
            variables.hasReset = true;
            variables.currentLayoutData = {};
            $.each(BaseGrid.ids, function(i, val) {
                $(val).jqxGrid("loadstate", BaseGrid.layouts[val]);
            });
        }
    };
